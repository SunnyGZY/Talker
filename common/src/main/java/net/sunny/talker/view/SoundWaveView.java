...
public class SoundWaveView extends View {
    ...
    public SoundWaveView(Context context, AttributeSet attr) {
        super(context, attr);
        init(context);
    }

    public SoundWaveView(Context context, AttributeSet attr, int style) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < (waveAmount * 2); i++) {
            ...
            if (i % 2 == 0) {
                h = (canvas.getHeight() / 2 + canvas.getHeight() / 2 * waveHeight);
            } else {
                h = (canvas.getHeight() / 2 - canvas.getHeight() / 2 * waveHeight);
            }
        ...
        for (int i = 0; i < (waveAmount * 2); i++) {
            ...
            if (i % 2 == 0) {
                h2 = (canvas.getHeight() / 2 + canvas.getHeight() / 2 * waveHeight);
            } else {
                h2 = (canvas.getHeight() / 2 - canvas.getHeight() / 2 * waveHeight);
            }
            ...
            path.moveTo(x1 + canvas.getWidth() * progress / (20 * waveAmount),
                    canvas.getHeight() / 2);
            path.quadTo(x2 + canvas.getWidth() * progress / (20 * waveAmount),
                    h2 / 2, x3 + canvas.getWidth() * progress / (20 * waveAmount),
                    canvas.getHeight() / 2);
        }
        canvas.drawPath(path, paint);
    }
