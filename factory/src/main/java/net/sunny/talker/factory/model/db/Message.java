...
public class Message extends BaseDbModel<Message> implements Serializable {
    ...
    /**
     * 复制一个属性一样的SessionMessage
     *
     * @return 复制完成的Message
     */
    public Message copy() {
        Message copyMessage = new Message();
        copyMessage.id = this.id;
        copyMessage.content = this.content;
        copyMessage.attach = this.attach;
        copyMessage.type = this.type;
        copyMessage.createAt = this.createAt;
        copyMessage.status = this.status;
        copyMessage.group = this.group;
        copyMessage.sender = this.sender;
        copyMessage.receiver = this.receiver;
